...
namespace RPA_Workbench.Views.WFDesigner
{
    public class RobotInstance : IOpenRPAClient
    {
        ...
        public static RobotInstance instance
        {
            get
            {
                if (_instance == null)
                {
                    ...
                    RPA.Workbench.Interfaces.IPCService.OpenRPAServiceUtil.InitializeService();
                }
                return _instance;
            }
        }
        ...
        public IDesigner[] Designers
        {
            get
            {
                if (Window == null) return new RPA_Workbench.Views.WFDesigner.WFDesigner[] { };
        ...
        public async Task LoadServerData(bool force)
        {
            ...
            try
            {
                if (ProjectCount == 0)
                {
                    ...
                    var projects = await global.webSocketClient.Query<Project>("openrpa", "{_type: 'project'}", orderby: "{\"name\":-1}", top: Config.local.max_projects);
                    projects = projects.OrderBy(x => x.name).ToArray();
                    var projectids = projects.Select(x => "'" + x._id + "'").ToArray();
                    var q = "{ _type: 'workflow', 'projectid': {'$in': [" + string.Join(",", projectids) + "]}}";
                    var workflows = await global.webSocketClient.Query<Workflow>("openrpa", q, orderby: "{\"projectid\":-1,\"name\":-1}", top: Config.local.max_workflows);
                    ...
                    var detectors = await global.webSocketClient.Query<RPA.Workbench.Interfaces.entity.Detector>("openrpa", "{_type: 'detector'}");
                    ...
                    foreach (var d in detectors)
                    {
                        ...
                        d.Path = RPA.Workbench.Interfaces.Extensions.ProjectsDirectory;
                    ...
                    foreach (var p in projects)
                    {
                        ...
                        p.Path = System.IO.Path.Combine(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory, p.name);
                        if (folders.Contains(p.Path))
                        {
                            p.Path = System.IO.Path.Combine(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory, p._id);
                        }
                        folders.Add(p.Path);
                    }
                    ...
                    foreach (var wf in workflows)
                    {
                        ...
                        if (hasProject == null)
                        {
                            if (up == null) up = await Project.Create(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory, "Unknown", false);
                            wf.Project = up;
                            up.Workflows.Add(wf);
                        }
                    }
                ...
                else if (!global.openflowconfig.supports_watch || force)
                {
                    ...
                    GenericTools.RunUI(async () =>
                    {
                        ...
                        {
                            project.Path = System.IO.Path.Combine(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory, project.name);
                        ...
                        foreach (var workflow in workflows)
                        {
                            ...
                            {
                                if (!(RobotInstance.instance.GetWorkflowDesignerByIDOrRelativeFilename(workflow.IDOrRelativeFilename) is RPA_Workbench.Views.WFDesigner.WFDesigner designer))
                                {
                                    int index = -1;
                                    try
                                    {
        ...
        public void ParseCommandLineArgs(IList<string> args)
        {
            ...
            try
            {
                ...
                {
                    RPA.Workbench.Interfaces.IPCService.OpenRPAServiceUtil.RemoteInstance.RunWorkflowByIDOrRelativeFilename(options["workflowid"].ToString(), false, options);
                }
            }
            catch (Exception ex)
            {
        ...
        public async Task init()
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(Config.local.wsurl))
                {
                    ...
                    var Detectors = RPA.Workbench.Interfaces.entity.Detector.loadDetectors(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory);
                    foreach (var d in Detectors)
                    {
                        IDetectorPlugin dp = null;
                        d.Path = RPA.Workbench.Interfaces.Extensions.ProjectsDirectory;
            ...
            try
            {
                ...
                else
                {
                    ...
                    var _Projects = await Project.LoadProjects(RPA.Workbench.Interfaces.Extensions.ProjectsDirectory);
        ...
        private async void RobotInstance_WebSocketClient_OnOpen()
        {
            ...
            RPA.Workbench.Interfaces.entity.TokenUser user = null;
            try
            {
                string url = "http";
                            ...
                            try
                            {
                                ...
                                GenericTools.RunUI(async () =>
                                {
                                    try
                                    {
                                      ...
                                      //  var signinWindow = new Views.SigninWindow(url, true); //COMMENTED BY RUWE
                                        Log.Debug("ShowDialog " + string.Format("{0:mm\\:ss\\.fff}", sw.Elapsed));
                                        //signinWindow.ShowDialog(); // COMMENTED BY RUWE
                                        //jwt = signinWindow.jwt;// COMMENTED BY RUWE
        ...
        async private Task RegisterQueues()
        {
            ...
            try
            {
                ...
                RPA.Workbench.Interfaces.entity.TokenUser user = global.webSocketClient.user;
                try
                {
                    if (RPA.Workbench.Interfaces.win32.ChildSession.IsChildSessionsEnabled())
                    {
                        var CurrentP = System.Diagnostics.Process.GetCurrentProcess();
                ...
                if (registerqueues)
                {
                    ...
                    {
                        var roles = await global.webSocketClient.Query<RPA.Workbench.Interfaces.entity.apirole>("users", "{_id: '" + role._id + "'}", top: 5000);
        ...
        private async void WebSocketClient_OnClose(string reason)
        {
            ...
            if (autoReconnect)
            {
                try
                {
                    ...
                    global.webSocketClient = new RPA.Workbench.Net.WebSocketClient(Config.local.wsurl);
                    global.webSocketClient.OnOpen += RobotInstance_WebSocketClient_OnOpen;
                    global.webSocketClient.OnClose += WebSocketClient_OnClose;
                    global.webSocketClient.OnQueueMessage += WebSocketClient_OnQueueMessage;
                    global.webSocketClient.OnQueueClosed += WebSocketClient_OnQueueClosed;
        ...
        private async void WebSocketClient_OnQueueMessage(IQueueMessage message, QueueMessageEventArgs e)
        {
            ...
            RPA.Workbench.Interfaces.mq.RobotCommand command = null;
            try
            {
                command = Newtonsoft.Json.JsonConvert.DeserializeObject<RPA.Workbench.Interfaces.mq.RobotCommand>(message.data.ToString());
                ...
                if (command.command == "invoke" && !string.IsNullOrEmpty(command.workflowid))
                {
                    ...
                    lock (statelock)
                    {
                        ...
                        GenericTools.RunUI(() =>
                        {
                            ...
                            try
                            {
                                if (RobotInstance.instance.GetWorkflowDesignerByIDOrRelativeFilename(command.workflowid) is RPA_Workbench.Views.WFDesigner.WFDesigner designer)
                                {
            ...
            {
                command = new RPA.Workbench.Interfaces.mq.RobotCommand
                {
                    command = "error",
            ...
            System.Windows.MessageBoxButton.YesNo, System.Windows.MessageBoxImage.None, System.Windows.MessageBoxResult.Yes);
                    ...
                    else if (exists == null)
                    {
                        GenericTools.RunUI(() =>
                        {
                            try
                            {
                                if (instance.Window.SelectedContent is RPA_Workbench.Views.WFDesigner.WFDesigner designer)
                                {
                        ...
                        try
                        {
                            if (exists != null && exists._version != project._version)
                            {
                                ...
                                try
                                {
                                    ...
                                    exists.dependencies = project.dependencies;
                                    exists.disable_local_caching = project.disable_local_caching;
                                    exists._acl = project._acl;
                                    exists._encrypt = project._encrypt;
                                    exists._modified = project._modified;
                                    exists._modifiedby = project._modifiedby;
                ...
                if (_type == "detector")
                {
                    var d = Newtonsoft.Json.JsonConvert.DeserializeObject<RPA.Workbench.Interfaces.entity.Detector>(data["fullDocument"].ToString());
                    GenericTools.RunUI(() =>
                    {
                        try
                        {
